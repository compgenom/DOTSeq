...
Traditional gene-level approaches, e.g., annota2seq available at 
Bioconductor [@oertlin2019generally], obscure these regulatory layers. 
...
`DOTSeq` builds on the Bioconductor ecosystem, particularly `Bioc.gff` 
for parsing GTF files [@lawrence2025biocgff] and the 
`SummarizedExperiment` container for storing count matrices and 
metadata [@morgan2025summarized].
...
[@emmeans] and `ashr` [@stephens2017false] (`testDOU()`). 
...
the interactionResults slots:

```{r posthoc}
results <- interactionResults(dot)
results
```
...
```{r merge}
rdou <- results$DOU[results$DOU$contrast == "Mitotic_Cycling - Interphase", ]
rdte <- results$DTE[results$DTE$contrast == "Mitotic_Cycling - Interphase", ]

results <- merge(rdou, rdte, by = c("orf_id", "contrast"), all = TRUE)
```
...
order <- c("Mitotic_Cycling", "Mitotic_Arrest", "Interphase")
