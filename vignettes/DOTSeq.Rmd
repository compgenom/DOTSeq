...
(see [Prepare annotations](#annotation)). This pre-processing step 
...
# Prepare annotations {#annotation}
...
[ORFik](https://www.bioconductor.org/packages/release/bioc/html/ORFik.html)'s 
engine [@tjeldnes2021orfik]. This approach creates a `GRanges` object.

```{r get-orfs, eval = FALSE}
...
package. We can optionally clean up the BAM files before read counting 
by discarding reads mapped to introns, noncoding genes, and intergenic 
regions. Note that this dataset is not derived from a Ribo-seq experiment 
and is used solely for illustrative purposes. It should not be used for 
biological interpretation.

```{r pasilla, eval = FALSE}
...
tables as input for `DOTSeqDataSetsFromSO()`. This is alternative to 
