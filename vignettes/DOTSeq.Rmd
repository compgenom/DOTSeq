...
`SummarizedExperiment` data structure for storing count matrices and 
metadata.
...
[@ly2024nuclear]. This dataset includes snapshots of ribosome footprints 
...
The data frames below summarise differential translation results 
for each ORF. Key statistics include:
...
The DTE effect size (`log2FoldChange`) was derived from `lfcShrink()` 
within the `DOTSeq()` main function. We specify `type = ashr` to apply 
adaptive shrinkage and compute all pairwise contrasts between conditions. 
...
illustrate the distribution of effects for each metric. Spearman 
