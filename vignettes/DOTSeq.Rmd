...
if (requireNamespace("eulerr", quietly = TRUE)) {
    plotDOT(plot_types = "venn", results = results, force_new_device = FALSE)
} else { 
...
the DOTSeq beta-binomial GLM. This plot combines the probabilistic 
estimates of ORF usage with post hoc statistical significance 
annotations, allowing interpretation of DOU across conditions.

```{r plot-orf-usage, eval = requireNamespace("ggsignif", quietly = TRUE), fig.small = TRUE}
gene_id <- "NUP50"
usage_order <- c("Mitotic_Cycling", "Mitotic_Arrest", "Interphase")

if ((requireNamespace("ggplot2", quietly = TRUE)) && (requireNamespace("ggsignif", quietly = TRUE))) {
    plotDOT(
        plot_types = "usage",
        sumExp = m$sumExp, 
        gene_id = gene_id, 
    ...
    message("Package 'ggsignif' not installed; skipping ORF Usage plot.")
}
```
...
In addition to gene symbols, the `plotDOT()` function supports querying 
using gene IDs in the rowData/rowRanges. This enable us to generate ORF 
usage plots using stable gene identifiers, which is especially useful 
when gene symbols are ambiguous.

```{r plot-orf-usage-eif4g1, eval = requireNamespace("ggsignif", quietly = TRUE), fig.small = TRUE}
gene_id <- "ENSG00000114867"
usage_order <- c("Mitotic_Cycling", "Mitotic_Arrest", "Interphase")
