...
#'
#' @description
#' This function takes an R formula object that may contain random effect terms
#' and returns a new formula with all such terms removed.
#'
#' @param formula An R formula object. Random effect terms are identified by the
#'        pattern \code{(1 | group)}.
#'
...
#'
#' @param formula_input A formula object (e.g.,
...
#'
#' @return A reduced formula object that excludes terms with only one level in
...
reduce_formula <- function(formula_input, data) {
    ...
    if (length(matched_terms) > 1) {
        stop(
            "Expected one interaction term (e.g., condition * strategy), but got multiple: ",
